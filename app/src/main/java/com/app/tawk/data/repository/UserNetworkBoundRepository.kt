...
abstract class UserNetworkBoundRepository<RESULT, REQUEST> {
    ...
    fun asFlow() = flow<Resource<RESULT>> {
       ...
       emit(Resource.Success(fetchFromLocal().first()))
        ...
        if (apiResponse.isSuccessful && remoteUsers != null) {
            // Save userDetail into the persistence storage
        ...
        // Retrieve user detail from persistence storage and emit
        emitAll(
            fetchFromLocal().map {
        ...
        e.printStackTrace()
        emit(Resource.Failed("Network error! Can't get User detail."))
    }
